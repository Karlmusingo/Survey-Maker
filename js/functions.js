$(document).ready(function () {
    ...
    $('#add').click(function () {
        i++;
        $('#addA').append('<div class="input-group" id="answer' + i + '">\n\
...
//                console.log(data);
    ...
    $('#submit').click(function () {
        $.ajax({
            ...
            success: function (data) {
                ...
                var result = "<table class='table table-bordered table-responsive text'><tr></tr>"
                $.each(obj, function () {
                    i++;
                    $this = this;
                    result = result + "<tr> <td> " + i + " </td><td>" + $this.question + "</td><td>";
                    var rep = "<table class='table table-bordered table-responsive text-left'>";
                    for (var id in $this.reponses) {
                        j++;
                        rep = rep + "<tr><td>" + j + "</td><td>" + $this.reponses[id].valeur + "</td></tr>";
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.each(obj, function () {
                    ...
                    $this = this;
                    result = result + "<tr> <td> " + i + " </td><td>" + $this.question + "</td><td>";
                    var rep = "<table class='table jumbottron table-responsive text-left'>";
                    for (var id in $this.reponses) {
                        j++;
                        rep = rep + "<tr><td>" + j + "</td><td>" + $this.reponses[id].valeur + "</td></tr>";
    ...
    $('button[data-toggle=modal]').click(function () {
        ...
        $('#sr-only').html('<input type="text" name="idSection" value="' + idSection + '"/>');
    });
    ...
    //            data: $('#form').serialize(),
                success: function (data)
                {
                    ...
                    $.each(reponses, function () {
                        optionAppend = optionAppend + "<option value=" + this.id_question + ">" + this.question + "</option>";
                    });
...
//            data: $('#form').serialize(),
            success: function (data)
            {
                ...
                var optionAppend = "";
                $.each(reponses, function () {
                    optionAppend = optionAppend + "<option value=" + this.id_reponse_proposee + ">" + this.valeur + "</option>";
                });
   ...
   $('input').click(function (){
       ...
       var question_id_clicked = $(this).parent().parent().attr('id');
...
//       console.log(class_hidden+$(this).val());
   }); 
